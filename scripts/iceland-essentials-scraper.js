...
class IcelandEssentialsScraper {
  constructor() {
  ...
  async getIcelandProductDetails(browser, productId, productName) {
    ...
    try {
      ...
      const productDetails = await page.evaluate(() => {
        ...
        // Image - Extract from official CDN (assets.iceland.co.uk)
        const imageSelectors = [
          ...
          '.product-gallery img',
          'img[src*="product"]',
          'img[src*="assets.iceland.co.uk"]',
          '.product-tile img',
          '.product-card img'
        ];
        
        // First pass: Look for Iceland CDN images
        for (const selector of imageSelectors) {
          const imgEl = document.querySelector(selector);
          if (imgEl && imgEl.src && imgEl.src.includes('assets.iceland.co.uk')) {
  ...
  async saveToLocalFile(products) {
    try {
      ...
      const filename = `iceland-essentials-${timestamp}.json`;
      ...
      console.log(`\n💾 Iceland essentials saved to: ${filepath}`);
  ...
  async testWithApple() {
    console.log('🧪 TESTING ICELAND WITH APPLE');
...
// Test function
async function main() {
  const scraper = new IcelandEssentialsScraper();
...
module.exports = IcelandEssentialsScraper;
